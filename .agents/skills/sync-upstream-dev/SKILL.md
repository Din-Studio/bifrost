...
The user's instructions take precedence over this workflow. A request to sync `upstream/dev` into `origin/dev` authorizes the normal end-to-end flow below, including the final push to `origin/dev`. Respect narrower requests such as inspect-only, local-only, prepare-only, or no-push.
